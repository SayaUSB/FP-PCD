...
def apply_otsu(img: np.ndarray, fov_mask: np.ndarray, thresh_scale: float = 0.92) -> np.ndarray:
...
def morphological_postprocess(binary: np.ndarray, min_area: int = 120) -> np.ndarray:
    bool_img = binary > 0
    selem = disk(1)
